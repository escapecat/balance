...
(function () {
  ...
  /* ⚠️ **别再写填底部空白的代码了。**
   *
   *  这里前后出现过 fillRest、min-height:100svh、
   *  最后一块列表 flex:1 铺到底 —— 全都删了，一个都没有用。
   *
   *  因为 tab 栏底下那块空白 **不在页面里**：
   *    浏览器里它是底部工具栏（前进/后退/标签页），
   *    standalone 里它是 env(safe-area-inset-bottom)，home 条的避让区。
   *  两者都在视口**外面**，CSS 够不着；后者还必须留 ——
   *  不留的话最下排按钮会被系统手势吃掉。
   *
   *  真正的解法在 style.css：给 .tabbar 一个和页面不同的底色。
   *  那块避让区于是看着是「栏的一部分」而不是「页面下面空了一块」。
   *  原生 app 留的是同样一块，区别只在有没有边界。 */
