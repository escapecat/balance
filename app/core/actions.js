...
var Actions = (function () {
  ...
  // 只有两种动作:**买入**和**卖出**。
  //
  // ⚠️ **现金分红记成卖出就行** —— 两者在数学上完全等价:
  //      分红 X:  inflow = 现金变化 − X    market = 持仓变化 + X
  //      卖出 X:  inflow = 现金变化 − X    market = 持仓变化 + X
  //    分红那个类型只多带了一点语义(不是你主动卖的),
  //    而为了这点语义要在每次记账时多面对一个选项,不划算。
  //    (`dividend` 的处理逻辑在下面留着,哪天真需要区分再加回入口。)
  //
  // ⚠️ **「记个决定」这种纯留痕的也砍了。** 不影响任何计算的东西
  //    摆在动作列表里,只会让你每次都要跳过它。
  //
