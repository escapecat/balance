...
//    有限额的类今天只吃得下 2,000/3,000,分给它一大笔钱是**假的**,
//    那笔钱实际还躺在现金里。而无限额的黄金因为「按比例」只分到 36,149,
//    不是该有的 44,365。
...
// 对的做法:债一次拿满 112.5k,股走按日投 —— 它今天本来也只吃得下 2,000。
...
ok(bond && Math.abs(bond.amount - 112500) < 2,
   '★ 无限额的债应该一次拿满 112,500,实际 ' + (bond || {}).amount +
...
ok(soldS <= 30000 + 1,
   '★ 卖掉了锁仓的部分(卖 ' + soldS + ',可卖只有 30,000)—— 这份清单没法照着做');
